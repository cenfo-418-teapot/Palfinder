...
class GroupSwipeCallback(private val activity: AppCompatActivity): ItemTouchHelper.SimpleCallback(
    0,
...
//    private val background: ColorDrawable = ColorDrawable(Color.parseColor("#" +R.color.error.toHexString()))
    ...
    override fun onSwiped(viewHolder: RecyclerView.ViewHolder, swipeDir: Int) {
        ...
        val group = GroupAdmin.getGroup(position)
        // async remove from backend
        GroupService.deleteGroup(group)
        if (group?.imageName != null) {
            //asynchronously delete the image (and assume it will work)
            GroupService.deleteImage(group.imageName!!)
        }
        GroupAdmin.deleteGroup(position)
